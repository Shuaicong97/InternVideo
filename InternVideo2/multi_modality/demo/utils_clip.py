...
def images_to_numpy(image_folder):
...
def setup_internvideo2(config: dict):
    if "bert" in config.model.text_encoder.get("name", ""):
        tokenizer = BertTokenizer.from_pretrained('/nfs/data3/shuaicong/InternVideo2/demo_weights/bert-large-uncased',
                                                  local_files_only=True)
        model = InternVideo2_CLIP(config=config, tokenizer=tokenizer, is_pretrain=True)
    else:
        model = InternVideo2_CLIP(config=config, is_pretrain=True)
...
class InternVideo2_CLIP(nn.Module):
    ...
    def load_checkpoint(self, vision_ckpt_path=None, text_ckpt_path=None, extra_ckpt_path=None):
        ...
        if self.config.model.get('load_vision_ckpt_from_internvideo2_stage2', False):
            from models.backbones.internvideo2.pos_embed import interpolate_pos_embed
            orig_t_size = self.config.model.get('vision_ckpt_t_size', 4)
            interpolate_pos_embed(vision_ckpt, self.vision_encoder,
                                  orig_t_size=orig_t_size)  # 4 for InternVideo2 stage2
